...
def xplot_colortable(colors, ncols=4, sort_colors=True):
    ...
    for i, name in enumerate(names):
        ...
        ax.add_patch(
            plt.Rectangle(
                (xi_swatch, y * cell_h), swatch_w, swatch_h, facecolor=color, edgecolor="0.8"
            )
        )
    fig.tight_layout()
    return fig, ax
...
blue_ombre = LinearSegmentedColormap.from_list(
    "blue_ombre", ["#001F3F", "#005B96", "#0074D9", "#7FDBFF", "#E0F7FF"]
)
