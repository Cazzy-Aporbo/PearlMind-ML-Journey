...
DATA_PATH = Path("/mnt/data/databricks-dolly-15k.jsonl")
...
def load_dolly(path: Path) -> pd.DataFrame:
    ...
    expected_cols = {"instruction", "context", "response", "category"}
...
def top_tokens(texts, k=20):
    # lightweight token frequency for exploration
    freq = {}
    for t in texts:
...
def main():
    ...
    bar_counts(counts, "Records per Category", ASSETS_DIR / "category_counts.png", color_key="lavender", edge_key="lavender_edge")
    means = per_category_means(df)
    bar_means(means["inst_len_mean"].to_dict(), "Mean Instruction Length by Category", ASSETS_DIR / "inst_len_means.png", color_key="teal", edge_key="teal_edge", ylabel="Chars")
    bar_means(means["resp_len_mean"].to_dict(), "Mean Response Length by Category", ASSETS_DIR / "resp_len_means.png", color_key="blue", edge_key="teal_edge", ylabel="Chars")
    # Token peek for creative vs. closed QA
    ...
    creative_top = top_tokens(sample_creative["response"].tolist(), k=25)
    closed_top = top_tokens(sample_closed["response"].tolist(), k=25)
    (ASSETS_DIR / "creative_top_tokens.json").write_text(json.dumps(creative_top, indent=2))
    (ASSETS_DIR / "closedqa_top_tokens.json").write_text(json.dumps(closed_top, indent=2))
    print("Analysis complete. Assets written to:", ASSETS_DIR)
